...
import { insertCompteCourantSchema, type CompteCourant } from "@shared/schema";
...
export default function AddCompteCourant() {
  ...
  const createCompteMutation = useMutation({
    mutationFn: async (data: z.infer<typeof formSchema>) => {
      ...
      if (!response.ok) {
        ...
        throw new Error(errData.error || "Failed to create compte courant");
      }
    ...
    onError: () => {
      toast({
        title: "Erreur",
        description: "Échec de la création du compte courant",
        variant: "destructive",
      });
    },
  });
  ...
  const onSubmit = (data: z.infer<typeof formSchema>) => {
    createCompteMutation.mutate(data);
  };
      ...
      <div className="p-4">
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
            ...
            >
              {createCompteMutation.isPending ? "Création..." : "Créer le compte courant"}
            </Button>
          </form>
        </Form>
      </div>
